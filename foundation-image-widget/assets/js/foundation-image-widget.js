...
window.FoundationImageWidget = window.FoundationImageWidget || {};

(function( window, $, _, Backbone, wp, undefined ) {
	'use strict';

	var Control, l10n,
		FoundationImageWidget = window.FoundationImageWidget,
		Attachment = wp.media.model.Attachment,
		finder = { view: {} },
		frames = [];

	// Link any localized strings.
	l10n = FoundationImageWidget.l10n = FoundationImageWidget.l10n || {};
	...
	Control = function( el, options ) {
		...
		selector = this.$el.data( 'target' ) || '.foundation-image-widget-control-target';
		...
		this.frame = function() {
			...
			frame.state( 'library' ).on( 'select', function() {
				...
				frame.control.$el.trigger( 'selectionChange.foundationimagewidget', [ selection ] );
			});
	...
	_.extend( FoundationImageWidget, {
		/**
		...
		getControl: function( el ) {
			...
			$control = $( el ).closest( '.foundation-image-widget-control' );
	...
	finder.view.Page = wp.Backbone.View.extend({
		events: {
			'click .foundation-image-widget-find-posts-button' : 'openModal'
		},
		...
		openModal: function( e ) {
			var $target = $( e.target).closest( '.foundation-image-widget-input-group' ).find( 'input[type="text"]' );
			e.preventDefault();
	...
	finder.view.Modal = wp.Backbone.View.extend({
		tagName: 'div',
		className: 'find-box',
		template: wp.template( 'foundation-image-widget-modal' ),

		events : {
			'click .foundation-image-widget-modal-search-button': 'findPosts',
			'keypress .foundation-image-widget-modal-search-field': 'routeKey',
			'keyup .foundation-image-widget-modal-search-field': 'routeKey',
			'click tr': 'select',
		...
		render: function() {
			...
			this.$field = this.$el.find( '.foundation-image-widget-modal-search-field' );
			this.$overlay = $( '.ui-find-overlay' );
			this.$response = this.$el.find( '.foundation-image-widget-modal-response' );
			this.$spinner = this.$el.find( '.foundation-image-widget-modal-search .spinner' );
			return this;
		},
		...
		findPosts: function() {
			...
			wp.ajax.post( 'foundation_image_widget_find_posts', {
				s: self.$field.val(),
	...
	$( document ).ready(function() {
		...
		$body.on( 'click', '.foundation-image-widget-control-choose, .foundation-image-widget-form img', function( e ) {
			e.preventDefault();
			FoundationImageWidget.getControl( this ).frame().open();
		});

		// Update the image preview and size dropdown in a widget when an image is selected.
		$body.on( 'selectionChange.foundationimagewidget', '.foundation-image-widget-control', function( e, selection ) {
			var $control = $( e.target ),
				$sizeField = $control.closest( '.foundation-image-widget-form' ).find( 'select.image-size' ),
			...
			if ( $sizeField.length ) {
				FoundationImageWidget.updateSizeDropdownOptions( $sizeField, sizes );
			}
			...
			$control.find( 'img' ).remove().end()
				.prepend( image )
				.addClass( 'has-image' )
				.find( 'a.foundation-image-widget-control-choose' ).removeClass( 'button-hero' );
		});

		// Wire up the toggle checkboxes in the screen options tab.
		$( '.foundation-image-widget-field-toggle' ).on( 'click', function() {
			var $this = $( this ),
				field = $this.val(),
				$hiddenFields = $( '.foundation-image-widget-field-toggle:not(:checked)' );

			$( '.foundation-image-widget-field-' + field ).toggleClass( 'is-hidden', ! $this.prop( 'checked' ) );

			$.ajax({
				url: ajaxurl,
				type: 'POST',
				data: {
					action: 'foundation_image_widget_preferences',
					hidden: $hiddenFields.map(function() { return this.value; }).get().join( ',' ),
					nonce: FoundationImageWidget.screenOptionsNonce
				},
				success: function( data ) {
					if ( 'nonce' in data ) {
						FoundationImageWidget.screenOptionsNonce = data.nonce;
					}
				}
			});
		});

		$body.on( 'focus', '.foundation-image-widget-input-group-field', function() {
			$( this ).parent().addClass( 'is-focused' );
		}).on( 'blur', '.foundation-image-widget-input-group-field', function() {
