 ...
 * @since   3.0.0
 */
class Foundation_Image_Widget extends WP_Widget {
	/**
	...
	public function __construct( $id_base = false, $name = false, $widget_options = array(), $control_options = array() ) {
		$id_base = ( $id_base ) ? $id_base : 'foundationimage'; // Legacy ID.
		$name    = ( $name ) ? $name : __( 'Image (Foundation)', 'foundation-image-widget' );

		$widget_options = wp_parse_args(
			$widget_options,
			array(
				'classname'                   => 'widget_foundationimage', 'custom_class', // Legacy class name.
				'customize_selective_refresh' => true,
				'description'                 => __( 'An image from your Media Library.', 'foundation-image-widget' ),
			)
		);
	...
	public function widget( $args, $instance ) {
		$cache    = (array) wp_cache_get( 'foundation_image_widget', 'widget' );
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'foundation_image_widget', array_filter( $cache ), 'widget' );
		}
	}
	...
	public function render( $args, $instance ) {
		 ...
		 */
		$inside = apply_filters( 'foundation_image_widget_output', '', $args, $instance, $this->id_base );
		...
		if ( $inside ) {
			...
			$data['image_size'] = $image_size = ( ! empty( $instance['image_size'] ) ) ? $instance['image_size'] : apply_filters( 'foundation_image_widget_output_default_size', 'medium', $this->id_base );
			$data['title'] = ( empty( $instance['title'] ) ) ? '' : $instance['title'];
			$data = array_merge( $instance, $data );
			$data = apply_filters( 'foundation_image_widget_template_data', $data );

			ob_start();
			$templates = $this->get_template_names( $args, $instance );

			$template_loader = new Foundation_Image_Widget_Template_Loader();
	...
	public function form( $instance ) {
		...
		$button_class = array( 'button', 'button-hero', 'foundation-image-widget-control-choose' );
		 ...
		 * @param string $id_base Widget type id.
		 */
		$fields = (array) apply_filters( 'foundation_image_widget_fields', $this->form_fields(), $this->id_base );
		?>

		<div class="foundation-image-widget-form">
			 ...
			 * @param string $id_base  Widget type id.
			 */
			do_action( 'foundation_image_widget_form_before', $instance, $this->id_base );
			?>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Title:', 'foundation-image-widget' ); ?></label>
			...
			<?php if ( ! is_foundation_image_widget_legacy() ) : ?>
				<p class="foundation-image-widget-control<?php echo ( $image_id ) ? ' has-image' : ''; ?>"
					data-title="<?php esc_attr_e( 'Choose an Image', 'foundation-image-widget' ); ?>"
					data-update-text="<?php esc_attr_e( 'Update Image', 'foundation-image-widget' ); ?>"
					...
					?>
					<input type="hidden" name="<?php echo esc_attr( $this->get_field_name( 'image_id' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'image_id' ) ); ?>" value="<?php echo absint( $image_id ); ?>" class="image-id foundation-image-widget-control-target">
					<a href="#" class="<?php echo esc_attr( join( ' ', $button_class ) ); ?>"><?php _e( 'Choose an Image', 'foundation-image-widget' ); ?></a>
				</p>
			<?php endif; ?>
					...
					switch ( $field ) {
						case 'image_size' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'image_size' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'image_size' ) ); ?>"><?php _e( 'Size:', 'foundation-image-widget' ); ?></label>
						...
						case 'link' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'link' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'link' ) ); ?>"><?php _e( 'Link:', 'foundation-image-widget' ); ?></label>
								<span class="foundation-image-widget-input-group">
									<input type="text" name="<?php echo esc_attr( $this->get_field_name( 'link' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'link' ) ); ?>" value="<?php echo esc_url( $instance['link'] ); ?>" class="foundation-image-widget-input-group-field">
									<button class="foundation-image-widget-find-posts-button foundation-image-widget-input-group-button dashicons dashicons-search"></button>
								</span>
							</p>
							<p class="<?php echo esc_attr( $this->siw_field_class( 'new_window' ) ); ?>" style="margin-top: -0.75em; padding-left: 2px">
								<label for="<?php echo esc_attr( $this->get_field_id( 'new_window' ) ); ?>">
									...
									<?php _e( 'Open in new window?', 'foundation-image-widget' ); ?>
								</label>
							</p>
							<?php
							break;
						...
						case 'link_classes' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'link_classes' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'link_classes' ) ); ?>"><?php _e( 'Link Classes:', 'foundation-image-widget' ); ?></label>
						...
						case 'link_text' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'link_text' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'link_text' ) ); ?>"><?php _e( 'Link Text:', 'foundation-image-widget' ); ?></label>
						...
						case 'text' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'text' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>"><?php _e( 'Text:', 'foundation-image-widget' ); ?></label>
						...
						default :
							 ...
							 * registered with the 'foundation_image_widget_fields'
							 * filter.
							 *
							 * @since 3.0.0
							 *
							 ...
							 */
							do_action( 'foundation_image_widget_field-' . sanitize_key( $field ), $instance, $this );
					}
				}
			}
			 ...
			 * @param string $id_base  Widget type id.
			 */
			do_action( 'foundation_image_widget_form_after', $instance, $this->id_base );
			?>

		</div><!-- /.foundation-image-widget-form -->
		<?php
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance = apply_filters( 'foundation_image_widget_instance', $instance, $new_instance, $old_instance, $this->id_base );
	...
	public function get_image_sizes( $image_id ) {
		$sizes = array( 'full' => __( 'Full Size', 'foundation-image-widget' ) );

		$imagedata = wp_get_attachment_metadata( $image_id );
		if ( isset( $imagedata['sizes'] ) ) {
			$size_names = Foundation_Image_Widget_Plugin::get_image_size_names();
	...
	public function flush_group_cache( $post_id = null ) {
		...
		wp_cache_delete( 'foundation_image_widget', 'widget' );
	}
	...
	protected function get_template_names( $args, $instance ) {
		...
		return apply_filters(
			'foundation_image_widget_templates',
			$templates,
			$args,
			$instance,
			$this->id_base
	...
	protected function siw_field_class( $id ) {
		$classes = array( 'foundation-image-widget-field', 'foundation-image-widget-field-' . sanitize_html_class( $id ) );

		$hidden_fields = Foundation_Image_Widget_Plugin::get_hidden_fields();
