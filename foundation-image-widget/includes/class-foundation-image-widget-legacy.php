 ...
 * @since 4.0.0
 */
class Foundation_Image_Widget_Legacy {
	/**
	 * Load legacy support.
	 *
	 * @since 4.0.0
	...
	public function load() {
		add_filter( 'foundation_image_widget_output', array( $this, 'output' ), 10, 4 );
		add_filter( 'foundation_image_widget_fields', array( $this, 'fields' ), 10, 2 );
		add_action( 'foundation_image_widget_field-legacy', array( $this, 'display_fields' ), 10, 2 );
		add_filter( 'foundation_image_widget_instance', array( $this, 'sanitize_data' ), 10, 4 );
	}
	...
	public function output( $output, $args, $instance, $id_base ) {
		if ( 'foundationimage' != $id_base || ! empty( $instance['image_id'] ) || empty( $instance['image'] ) ) {
			return $output;
		}
	...
	public function fields( $fields, $id_base ) {
		if ( 'foundationimage' == $id_base && is_foundation_image_widget_legacy() ) {
	...
	public function display_fields( $instance, $widget ) {
		if ( is_foundation_image_widget_legacy() || ! empty( $instance['image'] ) ) :
			?>
			<div class="foundation-image-widget-legacy-fields">
				<?php if ( ! is_foundation_image_widget_legacy() ) : ?>
					<p>
						<em><?php _e( 'These fields are here to maintain your data from an earlier version.', 'foundation-image-widget' ); ?></em>
					</p>
					<p>
						<em><?php _e( 'Select an image, then clear these values, and they will disappear when you save the widget.', 'foundation-image-widget' ); ?></em>
					</p>
				<?php endif; ?>

				<p>
					<label for="<?php echo esc_attr( $widget->get_field_id( 'image' ) ); ?>"><?php _e( 'Image URL:', 'foundation-image-widget' ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $widget->get_field_id( 'alt' ) ); ?>"><?php _e( 'Alternate Text:', 'foundation-image-widget' ); ?></label>
	 ...
	 *
	 * Called in Foundation_Image_Widget::update().
	 *
	 * @since 4.0.0
	 *
