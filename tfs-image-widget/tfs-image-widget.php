...
class TFS_Image_Widget extends WP_Widget {
	 ...
	 * 2) parent template, 3) plugin resources. will look in the tfs-image-widget/
	 * directory in a theme and the views/ directory in the plugin
	 *
	 * @param string $template template file to search for
	 * @return template path
	 * @author SteelBridge Media. (Chris Parsons)
	 **/
	...
	public function getTemplateHierarchy($template) {
		...
		return apply_filters( 'sp_template_tfs_image-widget_'.$template, $file);
	}
	...
	public function post_upgrade_nag() {
		...
		$version_key = '_tfssbm_img_version';
		if ( get_site_option( $version_key ) == self::VERSION ) return;

		$msg = sprintf(__('Thanks for upgrading the TFS Image Widget!'));
	...
	public function plugin_row_meta( $meta, $file ) {
		if ( $file == plugin_basename( dirname(__FILE__).'/tfs-image-widget.php' ) ) {
			$meta[] = '<span class="tfsimg-test">'.sprintf(__('tfs-image-widget')).'</span>';
		}
		return $meta;
	}
}
