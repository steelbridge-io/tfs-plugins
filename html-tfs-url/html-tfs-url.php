<?php
/*
Plugin Name: Add .html to end of TFS url
Plugin URI: http://steelbridgemedia.com
Description: Adds the option to include .html to the end of page URLs.
...
add_action('init', 'tfshtml_page_permalink', -1);
register_activation_hook(__FILE__, 'tfshtml_active');
register_deactivation_hook(__FILE__, 'tfshtml_deactive');

/* Original .html code manintains all presexisting .html URLs prior to Ver 2.0.0*/

function tfshtml_page_permalink() {
	global $wp_rewrite;
 if ( !strpos($wp_rewrite->get_page_permastruct(), '.html')){
		$wp_rewrite->page_structure = $wp_rewrite->page_structure . '.html';
 }
}
add_filter('user_trailingslashit', 'tfshtml_page_slash',66,2);
function tfshtml_page_slash($string, $type){
   global $wp_rewrite;
...
/**
 * Per-post/page options (Must Use Wordpress > 2.9)
 *
 * @package TFSCustomPermalinks
 * @since 0.1
 */
...
/**
 * Per-post options (For Wordpress < 2.9)
 *
 * @package TFSCustomPermalinks
 * @since 0.1
 */
