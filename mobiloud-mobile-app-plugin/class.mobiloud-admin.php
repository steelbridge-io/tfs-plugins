...
class Mobiloud_Admin {
	...
	public static function check_mailing_list_alert() {
		...
		if ( Mobiloud::get_option( 'ml_maillist_alert', '' ) === '' && Mobiloud::get_option( 'ml_initial_details_saved', '' ) === true ) {
			Mobiloud_Intercom::track( 'mailinglist_signup' );
	...
	public static function menu_get_init() {
		...
		if ( 2 === $active_step && '' === Mobiloud::get_option( 'ml_user_sitetype', '' ) ) { // very first request.
			self::track_settings_set( 'welcome_request' );
		}
				...
				*/
				if ( count( $_POST ) && check_admin_referer( 'ml-form-' . $tab ) ) {
					if ( isset( $_POST['ml_preview_upload_image'] ) ) {
						...
						if ( $logo !== Mobiloud::get_option( 'ml_preview_upload_image' ) ) {
							self::track_settings_set( empty( $logo ) ? 'logo_cleared' : 'logo_updated' );
						}
	...
	public static function menu_push() {
		...
		switch ( $tab ) {
			...
			case 'notifications':
				...
				self::track_page_view( 'push_notifications', '' );
				break;
		}
	}
	...
	*/
	...
	private static function track_settings_set( $tab ) {
		if ( Mobiloud::get_option( 'ml_initial_details_saved' ) ) {
			Mobiloud_Intercom::track( 'settings_' . $tab );
		}
	}

	private static function track_page_view( $tab, $prefix = 'settings_' ) {
		if ( Mobiloud::get_option( 'ml_initial_details_saved' ) ) {
			Mobiloud_Intercom::track( 'view_' . $prefix . $tab );
		}
	}
