...
class Forminator_Signature_Field extends Forminator_Field {
	...
	public function handle_sign_upload( $field ) {
		$this->field = $field;
		$id = $this->get_id( $field );
		$sign_field = isset( $_POST[ 'field-' . $id ] ) ? sanitize_text_field( $_POST[ 'field-' . $id ] ) : '';
		$name = self::get_signature_id( $id, $sign_field );

		// the data that comes from client side
		$sign_data = !empty( $_POST[ $name  . '_data' ] ) ? $_POST[ $name  . '_data' ] : ''; //phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		// the smooth data that comes from client side
		$sign_data_smooth = !empty( $_POST[ $name  . '_data_canvas' ] ) ? $_POST[ $name  . '_data_canvas' ] : ''; //phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
